...
class Select extends React.Component {
	...
	render() {
		...
		const input = css(mq({
			...
			cursor: (this.props.search ? 'auto' : 'pointer'),
			userSelect: (this.props.search ? 'auto' : 'none'),
			width:"100%",
			// height:"50px",
			fontSize:"1rem",
		...
		return (
			<div className={searchable+(this.props.className ? ' '+this.props.className : '')}>
				<input type="text" placeholder={this.props.placeholder} className={input}
					...
					readOnly={(this.props.search ? false : true)}/>
				<ul className={ul}>
					{this.renderOptions()}
				</ul>
			</div>
	...
	selectOption(iOption) {
		...
		if (this.props.onChange) this.props.onChange(option, iOption)
	}
	...
	getOptionValue(option) {
		...
		if (this.props.optionValue) {
			for (let ioptionValue = 0; ioptionValue < this.props.optionValue.length; ioptionValue++) {
